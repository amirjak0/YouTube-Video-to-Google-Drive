...
def get_gdrive_service():
    ...
    if not all([client_id, client_secret, refresh_token]):
        ...
        return None
    creds = Credentials(None, refresh_token=refresh_token, token_uri="https://oauth2.googleapis.com/token", client_id=client_id, client_secret=client_secret, scopes=["https://www.googleapis.com/auth/drive"])
    if not creds.valid:
...
def parse_video_id_and_quality(filename: str) -> Tuple[Optional[str], int]:
    ...
    if quality_matches:
        try: quality_height = int(quality_matches[-1])
        except ValueError: quality_height = 0
...
def scan_drive_folder(service, folder_id: str) -> Dict[str, Dict[str, Any]]:
    ...
    while True:
        try:
            ...
            res = service.files().list(q=query, fields="nextPageToken, files(id, name, size, createdTime)", pageSize=100, pageToken=page_token).execute()
            for item in res.get("files", []):
                name = item.get("name", "")
                vid_id, height = parse_video_id_and_quality(name)
                if vid_id:
                    existing[vid_id] = {"file_id": item.get("id"), "name": name, "height": height, "size": int(item.get("size") or 0)}
            page_token = res.get("nextPageToken")
            if not page_token: break
        except Exception as e:
            logger.error(f"Error reading Drive folder contents: {e}"); break
...
def upload_to_drive(service, file_path: Path, folder_id: str, old_file_id: Optional[str] = None) -> Optional[str]:
    ...
    try:
        ...
        if old_file_id and old_file_id != new_file_id:
            ...
            except Exception as del_err: logger.warning(f"Could not remove old file {old_file_id}: {del_err}")
        return new_file_id
    except Exception as e:
...
def verify_file_resolution(file_path: Path) -> int:
    try:
        cmd = ["ffprobe", "-v", "error", "-select_streams", "v:0", "-show_entries", "stream=height", "-of", "csv=p=0", str(file_path)]
        res = subprocess.run(cmd, capture_output=True, text=True, timeout=10)
        output = res.stdout.strip()
        if output.isdigit(): return int(output)
    except Exception as e: logger.debug(f"ffprobe check failed: {e}")
    return 0
...
def probe_best_format(video_url: str, cookies_path: Optional[str] = None) -> Tuple[Optional[str], int, str]:
    ...
    for client in PLAYER_CLIENT_CANDIDATES:
        ...
        try:
            ...
            info = json.loads(res.stdout); formats = info.get("formats", [])
            candidate_videos = [f for f in formats if f.get("vcodec") not in (None, "none") and int(f.get("height") or 0) > 0]
            if not candidate_videos: continue
            candidate_videos.sort(key=lambda x: (int(x.get("height") or 0), int(x.get("width") or 0), float(x.get("fps") or 0), float(x.get("tbr") or x.get("vbr") or 0)), reverse=True)
            client_max = candidate_videos[0]; client_height = int(client_max.get("height") or 0)
            ...
            if client_height > best_height:
                best_height, best_client, best_format_id = client_height, client, client_max.get("format_id")
            if best_height >= 1080: break
        except Exception as err: logger.debug(f"Client {client} probe error: {err}")
    if not best_format_id: best_format_id, best_height, best_client = "bestvideo+bestaudio/best", 1080, "ios"
...
def download_video(video_url: str, target_client: str, cookies_path: Optional[str] = None, max_resolution: int = 2160) -> Optional[Tuple[Path, int]]:
    ...
    for strat in strategies:
        ...
        cmd = ["yt-dlp","--no-playlist","--merge-output-format","mkv","-f",format_selector,"--js-runtimes","node","--extractor-args",f"youtube:player_client={strat['client']}","-o",output_template,"--embed-metadata","--no-mtime"]
        if strat["use_cookies"] and cookies_path and os.path.exists(cookies_path): cmd.extend(["--cookies",cookies_path])
        cmd.append(video_url)
        logger.info(f"Attempting download via {strat['desc']}...")
        res = subprocess.run(cmd, capture_output=True, text=True)
        if "The page needs to be reloaded" in res.stderr:
            logger.warning("Encountered YouTube 'The page needs to be reloaded' bot-check. Skipping to next client..."); continue
        if res.returncode != 0:
            logger.warning(f"Strategy {strat['desc']} failed (code {res.returncode}): {res.stderr[-300:] if res.stderr else ''}"); continue
        downloaded_files = list(DOWNLOADS_DIR.glob("*.mkv")) or list(DOWNLOADS_DIR.glob("*.*"))
        if not downloaded_files: continue
...
def process_single_video(video_url: str, gdrive_service, folder_id: str, existing_drive_files: Dict[str, Dict[str, Any]], update_existing: bool = True, cookies_path: Optional[str] = None, force_quality_limit: int = 2160):
    logger.info(f"\n=======================================================\nProcessing video: {video_url}")
    id_match = re.search(r"(?:v=|\/)([a-zA-Z0-9_-]{11})", video_url); video_id = id_match.group(1) if id_match else None
    ...
    existing_entry = existing_drive_files.get(video_id) if video_id else None; old_file_id = None
    if existing_entry:
        existing_height = existing_entry.get("height", 0); old_file_id = existing_entry.get("file_id"); old_name = existing_entry.get("name")
        ...
        if existing_height >= available_height and not os.environ.get("FORCE_RETRY"):
            logger.info(f"Video {video_id} is already in Drive at max resolution ({existing_height}p >= {available_height}p). Skipping."); return
        if update_existing and available_height > existing_height: logger.info(f">>> [AUTO-UPGRADE] Upgrading video {video_id} from {existing_height}p to {available_height}p! <<<")
        else: logger.info(f"Update existing is disabled. Skipping {video_id}."); return
    download_res = download_video(video_url, target_client=chosen_client, cookies_path=cookies_path, max_resolution=min(force_quality_limit, max(available_height, 1080)))
    if not download_res: logger.error(f"Failed to download high-resolution stream for {video_url}."); return
...
def get_playlist_videos(playlist_url: str) -> List[str]:
    if "list=" not in playlist_url: return [playlist_url]
    ...
    try:
        ...
        logger.error(f"Failed to extract playlist entries: {e}"); return [playlist_url]

def main():
    print("================================================================\nYouTube Video to Google Drive - High Quality Auto-Updater & Sync\n================================================================")
    ...
    if not playlist_url:
        logger.error("Error: YOUTUBE_PLAYLIST_URL or YOUTUBE_VIDEO_URL environment variable is required."); sys.exit(1)
    cookies_path = None
    if cookies_content and len(cookies_content.strip()) > 10:
        cookies_path = "youtube_cookies.txt"
        with open(cookies_path, "w", encoding="utf-8") as f: f.write(cookies_content)
    ...
    for idx, video_url in enumerate(video_urls, 1):
        ...
        try: process_single_video(video_url, service, folder_id, existing_drive_files, update_existing, cookies_path)
        except Exception as err: logger.error(f"An unexpected error occurred for {video_url}: {err}")
